...
void gameOfLife(Game &game) {
    mystd::vector<mystd::vector<int8_t>> oldHighway(game.lanes, mystd::vector<int8_t>(game.fragments, 0));
        ...
        for (std::size_t f = 0; f < game.fragments; ++f) {
            ...
            for (int dl = -1; dl <= 1; ++dl) {
                for (int df = -1; df <= 1; ++df) {
                    ...
                    if (nl >= 0 && nl < static_cast<int>(game.lanes) &&
                        nf >= 0 && nf < static_cast<int>(game.fragments)) {
...
void registerGameOfLife() {
    mods::registerMod(
        "Game of Life, Survive: " + std::to_string(survive) + ", Revive: " + std::to_string(revive) + ", Hold " + (hold_alive ? "Alive" : "Dead") + ", Before",
        &gameOfLife<survive, revive, hold_alive>,
        true
    );

    mods::registerMod(
        "Game of Life, Survive: " + std::to_string(survive) + ", Revive: " + std::to_string(revive) + ", Hold " + (hold_alive ? "Alive" : "Dead") + ", After",
        &gameOfLife<survive, revive, hold_alive>,
        false
    );
}

template<unsigned char i = 0, unsigned char j = 0>
struct RegisterAll {
    static void run() {
        if constexpr (i < 8) {
            if constexpr (j < 8) {

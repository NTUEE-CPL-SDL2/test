...
public:
    Renderer(Game& game_, int screenW_, int screenH_,
             TTF_Font* large_font_, TTF_Font* medium_font_, TTF_Font* small_font_)
        : game(game_), screenW(screenW_), screenH(screenH_),
          large_font(large_font_), medium_font(medium_font_), small_font(small_font_)
    {
    ...
    void render(SDL_Renderer* rnd) {
        ...
        for (size_t lane = 0; lane < game.lanes; ++lane) {
            ...
            for (size_t fragmentIdx = 0; fragmentIdx < game.fragments; ++fragmentIdx) {
                int8_t fragmentValue = game.highway[lane][fragmentIdx];
                uint64_t holdTime = 0;

                if (fragmentIdx == game.fragments - 1 && fragmentValue > 0 && lanePressed) {
                ...
                if (it == textureCache.end()) {
                    texture = createFragmentTexture(rnd, fragmentValue, lanePressed && fragmentIdx == game.fragments - 1, holdTime);
            ...
            drawText(rnd, keyHint, laneCenterX, screenH - 30, small_font, {200, 200, 200, 255}, ALIGN_CENTER);
        }
        ...
        if (game.combo > 0) {
            ...
            if (game.combo >= 50) {
                comboColor = {255, 215, 0, 255};
            } else if (game.combo >= 20) {
                comboColor = {255, 100, 255, 255};
            }

            drawText(rnd, comboText, screenW / 2, 100, large_font, comboColor, ALIGN_CENTER);

            drawText(rnd, "COMBO", screenW / 2, 180, medium_font, comboColor, ALIGN_CENTER);
        }

        std::string scoreText = "Score: " + std::to_string(game.score);
        drawText(rnd, scoreText, screenW / 2, 20, medium_font, {255, 255, 255, 255}, ALIGN_CENTER);

        int statsX = 10;
        int statsY = 50;
        int lineHeight = 30;

        drawText(rnd, "PERFECT: " + std::to_string(game.perfectCount), statsX, statsY, small_font, {0, 255, 0, 255});
        drawText(rnd, "GREAT: " + std::to_string(game.greatCount), statsX, statsY + lineHeight, small_font, {0, 200, 100, 255});
        drawText(rnd, "GOOD: " + std::to_string(game.goodCount), statsX, statsY + lineHeight * 2, small_font, {200, 200, 0, 255});
        drawText(rnd, "BAD: " + std::to_string(game.badCount), statsX, statsY + lineHeight * 3, small_font, {255, 100, 0, 255});
        drawText(rnd, "MISS: " + std::to_string(game.missCount), statsX, statsY + lineHeight * 4, small_font, {255, 0, 0, 255});
        ...
        drawText(rnd, timeText, screenW - 10, 20, small_font, {200, 200, 200, 255}, ALIGN_RIGHT);
    }
...
private:
    SDL_Texture* createFragmentTexture(SDL_Renderer* rnd, int8_t fragmentValue, bool pressed, uint64_t holdPressedTime) {
        SDL_Texture* texture = SDL_CreateTexture(rnd, SDL_PIXELFORMAT_RGBA8888,
        ...
        if (fragmentValue != 0) {
            SDL_Color borderColor = fragmentValue < 0 ?
                SDL_Color{255, 200, 200, 255} :
                SDL_Color{200, 255, 200, 255};

            SDL_SetRenderDrawColor(rnd, borderColor.r, borderColor.g, borderColor.b, borderColor.a);
            SDL_RenderDrawRect(rnd, &fillRect);

            if (fragmentValue > 0) {
                std::string holdText = std::to_string(fragmentValue);
                drawText(rnd, holdText, laneWidth / 2, fragmentHeight / 2,
                        small_font, {255, 255, 255, 255}, ALIGN_CENTER);
            }
        }
    ...
    SDL_Texture* getTextTexture(SDL_Renderer* rnd, const std::string& text, TTF_Font* font, SDL_Color color) {
    ...
    void drawText(SDL_Renderer* rnd, const std::string& text, int x, int y,
                  TTF_Font* font, SDL_Color color, TextAlignment align = ALIGN_LEFT) {
